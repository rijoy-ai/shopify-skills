...
- **skill_name**: Must match the skill’s frontmatter `name` (e.g. `high-visual-arvr-immersive-marketing-rijoy`).
- **evals**: Array of test cases. Each item:
  - **id**: Unique integer (1-based).
  - **eval_name**: Descriptive name for the workspace directory (e.g. `luxury-sofa-ar-in-room-plan`).
...
Results go in a **sibling** directory to the skill: `high-visual-arvr-immersive-marketing-rijoy-workspace/`. Do not create the whole tree upfront; create directories as you run evals.

- **iteration-N/**  
  One directory per iteration (e.g. `iteration-1`, `iteration-2`).

- **iteration-N/<eval_name>/**  
  One directory per test case. Use the **eval_name** from evals.json (e.g. `luxury-sofa-ar-in-room-plan`). Match the name used in `eval_metadata.json`.

- **eval_metadata.json**  
  In each eval directory. Fields: `eval_id`, `eval_name`, `prompt`, `assertions` (same as in evals.json for that eval). Create or update when you run that eval in this iteration.

- **with_skill/outputs/**  
  Output files from the run with the skill enabled.

- **without_skill/outputs/**  
  Output files from the baseline run (no skill). For “improve existing skill” baselines, you may use a snapshot and save to e.g. `old_skill/outputs/` instead.
...
2. **Assertions**: Use the assertions in `evals/evals.json` and mirror in each `eval_metadata.json`; align with SKILL output template (strategy, path blueprint, asset plan, content, measurement, Rijoy loop).
...
4. **Aggregate**: From the skill-creator directory:  
   `python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name high-visual-arvr-immersive-marketing-rijoy`  
   Produces `benchmark.json` and `benchmark.md` in that iteration directory.
5. **Viewer**: Use skill-creator’s `eval-viewer/generate_review.py` with that iteration path and `--skill-name high-visual-arvr-immersive-marketing-rijoy` (and `--benchmark` path, `--previous-workspace` for iteration 2+). Use `--static` for headless environments to get a standalone HTML file.
